...
function Avatar({ name, isAI, isGroup, online, size = 44 }) {
  const initials = !isAI && !isGroup && name
...
function CodeBlockPreview({ language, onOpen }) {
  return (
    <button
      ...
      style={{ background: "rgba(0,0,0,0.35)", border: `1px solid ${C.neon}44`, color: C.neon }}
      onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(0,0,0,0.5)")}
      onMouseLeave={(e) => (e.currentTarget.style.background = "rgba(0,0,0,0.35)")}
    >
      <Code2 size={16} />
      <span className="font-mono">{language}</span>
      <span className="ml-auto" style={{ color: C.textMuted }}>View code</span>
    </button>
  );
}
      ...
      <div className="flex items-start gap-2 max-w-[85%] md:max-w-[70%]">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold tracking-wide" style={{ color: C.neon }}>AI</span>
            <span className="text-[11px]" style={{ color: C.textMuted }}>{msg.time}</span>
          </div>
          <div className="rounded-2xl rounded-tl-sm px-4 py-3" style={{ background: C.neonSoft, border: `1px solid ${C.neon}33` }}>
            <p className="text-sm leading-relaxed" style={{ color: C.textPrimary }}>{msg.text}</p>
            {msg.type === "code" && (
              <CodeBlockPreview language={msg.language} onOpen={() => onOpenCode(msg)} />
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
      <div className="max-w-[80%] md:max-w-[60%]">
        ...
        {!isMe && msg.senderName && (
          <p className="text-xs mb-1 ml-1" style={{ color: C.violet }}>{msg.senderName}</p>
        )}
        <div
          className={`px-4 py-2.5 text-sm leading-relaxed ${isMe ? "rounded-2xl rounded-br-sm" : "rounded-2xl rounded-tl-sm"}`}
          style={{ background: isMe ? C.violet : C.elevated, color: isMe ? "#100420" : C.textPrimary }}
        >
          {msg.text}
        </div>
...
function ChatHeader({ contact, onBack }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ background: C.surface, borderBottom: `1px solid ${C.border}` }}>
      <button className="md:hidden -ml-1 p-1" onClick={onBack} aria-label="Back to chat list">
...
function MessageInput({ value, onChange, onSend }) {
  ...
  return (
    <div className="flex-shrink-0 px-3 md:px-4 py-3" style={{ background: C.surface, borderTop: `1px solid ${C.border}` }}>
      {mentioningAI && (
        <div className="flex items-center gap-1.5 mb-2 text-xs font-medium" style={{ color: C.neon }}>
          <Bot size={14} /> Asking AI
        </div>
      )}
      <div className="flex items-end gap-2">
        <button className="p-2 rounded-full flex-shrink-0" style={{ color: C.textMuted }} aria-label="Attach file">
          <Paperclip size={20} />
        </button>
        <div className="flex-1 flex items-end gap-2 rounded-2xl px-3 py-2" style={{ background: C.elevated, border: `1px solid ${mentioningAI ? C.neon + "55" : C.border}` }}>
          <textarea
            rows={1}
            value={value}
          ...
          />
          <button className="p-1 flex-shrink-0" style={{ color: C.textMuted }} aria-label="Emoji">
            <Smile size={20} />
          </button>
        </div>
        <button
          ...
          style={{ background: mentioningAI ? C.neon : C.violet, color: "#0a0a0c" }}
...
function CodePanel({ msg, onClose, onFeedback }) {
  ...
  return (
    <div className="fixed inset-0 z-50 md:static md:inset-auto md:z-auto md:w-[400px] md:flex-shrink-0 flex flex-col" style={{ background: C.surface, borderLeft: `1px solid ${C.border}` }}>
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ borderBottom: `1px solid ${C.border}` }}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-sm" style={{ color: C.textPrimary }}>{msg.language}</span>
        </div>
        <button onClick={onClose} className="p-1.5 rounded-full" style={{ color: C.textMuted }} aria-label="Close code panel">
          <X size={20} />
        </button>
      </div>
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <pre className="text-sm font-mono whitespace-pre-wrap rounded-xl p-4" style={{ background: "#000000", color: "#D9FFEA", lineHeight: 1.6 }}>
          {msg.code}
        </pre>
      </div>
      <div className="flex-shrink-0 px-4 py-3 flex items-center gap-2" style={{ borderTop: `1px solid ${C.border}` }}>
        <button onClick={handleCopy} className="flex-1 flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-medium transition-colors" style={{ background: C.violet, color: "#100420" }}>
          <Copy size={16} /> {copied ? "Copied!" : "Copy code"}
        </button>
        <button
          onClick={() => onFeedback(msg.id, true)}
          className="p-2.5 rounded-xl"
          style={{ background: msg.feedback === true ? C.neonSoft : C.elevated, color: msg.feedback === true ? C.neon : C.textMuted }}
        ...
        <button
          ...
          className="p-2.5 rounded-xl"
          style={{ background: msg.feedback === false ? "#3d1420" : C.elevated, color: msg.feedback === false ? C.hot : C.textMuted }}
...
function EmptyState({ onCreate }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 text-center">
      ...
      <p className="font-medium" style={{ color: C.textPrimary }}>No chats yet</p>
...
export default function WorkScreen() {
  ...
  const handleFeedback = (msgId, good) => {
    setMessagesByChat((prev) => ({
      ...prev,
      [activeId]: prev[activeId].map((m) => (m.id === msgId ? { ...m, feedback: good } : m)),
    }));
    if (codePanelMsg?.id === msgId) setCodePanelMsg((m) => ({ ...m, feedback: good }));
    // BACKEND HOOK: POST /api/ai/feedback { messageId: msgId, rating: good }
  };

  return (
    <div className="flex w-full h-screen overflow-hidden" style={{ background: C.void, fontFamily: "Inter, system-ui, sans-serif" }}>
      {/* ---------------- Sidebar / chat list ---------------- */}
      <div
      ...
      >
        <div className="flex items-center justify-between px-4 py-4 flex-shrink-0">
          <h1 className="text-lg font-semibold" style={{ color: C.textPrimary }}>Chats</h1>
          <button
        ...
        <div className="px-3 pb-3 flex-shrink-0">
          <div className="flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: C.elevated }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {chats.length === 0 ? (
            <p className="text-sm text-center mt-8 px-4" style={{ color: C.textMuted }}>
              No chats yet. Tap + to start one.
            </p>
          ) : (
            chats.map((c) => (
              <ContactRow key={c.id} contact={c} active={c.id === activeId} onClick={() => openChat(c.id)} />
            ))
          )}
        </div>
      </div>
      ...
      {/* ---------------- Chat window ---------------- */}
      <div className={`flex-1 min-w-0 flex-col ${mobileView === "chat" ? "flex" : "hidden md:flex"}`}>
        {activeContact ? (
          <>
            <ChatHeader contact={activeContact} onBack={() => setMobileView("list")} />
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 md:px-6 py-4 flex flex-col gap-4">
              {messages.length === 0 ? (
                <p className="text-sm text-center mt-8" style={{ color: C.textMuted }}>
                  No messages yet. Say hi!
                </p>
              ) : (
                messages.map((m) => <MessageBubble key={m.id} msg={m} onOpenCode={setCodePanelMsg} />)
              )}
            </div>
      ...
      {/* ---------------- Code panel ---------------- */}
      {codePanelMsg && (
        <CodePanel msg={codePanelMsg} onClose={() => setCodePanelMsg(null)} onFeedback={handleFeedback} />
      )}
    </div>
  );
}