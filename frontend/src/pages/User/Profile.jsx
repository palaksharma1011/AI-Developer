...
import {
  Mail, GitBranch, MapPin, Clock, Flame, FolderGit2, CheckCircle2,
  LogOut, Pencil, Star,
} from "lucide-react";
...
const stats = [
  { label: "Active projects", value: "4", icon: FolderGit2, accent: C.workflow },
  { label: "Tasks completed", value: "162", icon: CheckCircle2, accent: C.contribution },
...
export default function Profile() {
  return (
    <div className="min-h-screen w-full font-sans flex items-start justify-center px-4 py-8 sm:py-12" style={{ background: bg }}>
      <div className="w-full max-w-2xl">
        ...
        >
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${C.contribution}, ${C.collaboration})` }}
          />
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span
                  ...
                  style={{ background: `${C.collaboration}1A`, color: C.collaboration }}
                >
                  {user.role}
                </span>
              </div>
              <p className="text-xs text-neutral-500 mt-1">{user.team} · joined {user.joined}</p>
              <p className="text-sm text-neutral-400 mt-3 leading-snug">{user.bio}</p>
            </div>
            <button
        ...
        <div
          ...
          style={{ background: surface, border: `1px solid ${border}`, borderColor: border }}
        >
          <p className="text-[10px] uppercase tracking-wider text-neutral-500 pb-2">Details</p>
          <div className="divide-y" style={{ borderColor: border }}>
            <InfoRow icon={Mail} label="Email" value={user.email} />
            <InfoRow icon={GitBranch} label="GitBranch" value={user.github} />
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {user.skills.map((s) => (
              <span key={s} className="text-xs px-2.5 py-1 rounded-full bg-white/5 text-neutral-300">
                {s}
              </span>
            ))}
          </div>
        ...
        <button
          ...
          style={{ borderColor: `${C.collaboration}40`, color: C.collaboration }}
        >
          <LogOut size={15} />
          Log out
        </button>
